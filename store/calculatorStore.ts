...
export const useCalculatorStore = create<CalculatorState>()(
  persist(
    (set, get) => ({
      ...
      addIncome: (income) =>
        set((state) => ({
          additionalIncome: [...state.additionalIncome, { ...income, id: crypto.randomUUID() }]
        })),
      ...
      deleteIncome: (id) =>
        set((state) => ({
          additionalIncome: state.additionalIncome.filter((income) => income.id !== id)
        })),
      ...
      getTotalAdditionalIncome: () => {
        ...
        return additionalIncome.reduce((total, income) => total + income.amount, 0);
      },
